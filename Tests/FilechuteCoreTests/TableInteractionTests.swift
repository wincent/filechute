...
struct ArrowKeyTests {
  ...
  func upArrowNavigatesQL() {
    ...
    #expect(
      TableInteraction.reduce(key: .upArrow, context: ctx) == .navigateQuickLook(direction: -1))
  }
  ...
  func downArrowNavigatesQL() {
    let ctx = InteractionContext(isQuickLookVisible: true)
    #expect(
      TableInteraction.reduce(key: .downArrow, context: ctx) == .navigateQuickLook(direction: 1))
  }
